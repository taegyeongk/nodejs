...
create table `t_board_board`(
	`no` INT PRIMARY KEY AUTO_INCREMENT, -- 서버에 따라 자동생성
    `title` VARCHAR(1000) NOT NULL,
    `writer` VARCHAR(1000) NOT NULL,
    `content` VARCHAR(1000),
    `created_date` DATETIME NOT NULL,
    `updated_date` DATETIME DEFAULT current_timestamp() -- timestamp  = sysdate 처럼 현재 날짜를 입력함, 시간까지 포함한다.
);
...
create table `t_comment_board`(
    ...
    `bno` int not null,
    primary key (`no`) ,
    foreign key (`bno`) references `t_board_board`(`no`) ON UPDATE CASCADE
);
